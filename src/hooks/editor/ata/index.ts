import {
  ...
  export const setupTypeAcquisition = (config: ATABootstrapConfig) => {
    ...
    return (initialSourceFiles: string[]) => {
      estimatedToDownload = 0
      estimatedDownloaded = 0
  
      return resolveDeps(initialSourceFiles, 0).then(t => {
    ...
    async function resolveDeps(initialSourceFiles: string[], depth: number) {
      ...
      // Collect all the npm and DT DTS requests and getFiletreeForModuleWithVersionten their arrays
  ...
  export const getReferencesForModule = (ts: typeof import("typescript"), code: string, processedModules: Set<string> ) => {
    ...
    const referencesRaw = meta.referencedFiles
    ...
    for (const ref of referencesRaw) {
      ...
      if (!ref.fileName.startsWith(".")) {
        version = "latest"
        const line = code.slice(ref.end).split("\n")[0]!
        if (line.includes("// types:")) version = line.split("// types: ")[1]!.trim()
      }

      const processedModule = `${ref.fileName}@${version ?? ""}`
      if (processedModules.has(processedModule)) {
        continue
      }
      processedModules.add(processedModule)
  ...
  export function getNewDependencies(config: ATABootstrapConfig, moduleMap: Map<string, ModuleMeta>, code: string, processedModules: Set<string>) {
    const refs = getReferencesForModule(config.typescript, code, processedModules).map(ref => ({
      ...ref,
